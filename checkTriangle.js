...
const validateNumberInput = (...checkValue) => {
    const countWordList = ['pertama', 'kedua', 'ketiga'];
    checkValue.forEach((val, idx) => {
        if (typeof val !== 'number') {
            throw new ValidationError(`Argumen ${ countWordList.length < checkValue.length ? idx+1 : countWordList[idx]} harus number`);
        } 
    });
}
